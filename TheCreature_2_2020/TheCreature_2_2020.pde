...
float   eyeLeftRelativeX  = 5   ,  eyeLeftRelativeY  = 5;  //Vi ændre (int) til (float) for vi kan ikke gange en (float) med en (int) da (int) ikke involdvere kommatal  
...
void draw() {                         
  float ballwalk =  sin(ballX*0.5)*10;  //svingning på +10 til -10 pixels //gør ballwalk aktiv ved at fjerne kommentar tegnene (//)
  ballX         =  ballX  +  speedX;    //flytter min bold 
  ballY         =  ballY  +  speedY + (int)ballwalk;  // insætter ballwalk og gør den til int for at den kan blandes med ballY og speedY, og vi indsætter den her for at den er gældende for alle ellipser
